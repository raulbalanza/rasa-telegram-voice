...
class TelegramOutput(Bot, OutputChannel):
    ...
    async def _create_voice_response(self, text: Text):
    ...
    async def send_text_message(
        ...
        for message_part in text.strip().split("\n\n"):
            if message_type == 'audio':
                audio = await self._create_voice_response(message_part)
                if audio:                    
                    await self.send_voice(recipient_id, BytesIO(audio.read()))
                    audio.close()
                else:
    ...
    def blueprint(
        ...
        async def message(request: Request) -> Any:
            if request.method == "POST":
                ...
                out_channel.users[str(sender_id)].put(message_type)
                try:
...
class UserHistory:
    ...
    def get(self):
        try:
            element = self.queue.get(timeout=2)
        except queue.Empty:
            element = "text"
        return element

    def put(self, message_type):
        self.queue.put(message_type)